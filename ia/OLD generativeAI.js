...
class GenerativeAI {
    constructor(config, twitchClient) {
        ...
        this.huggingFaceApiUrl = "https://api-inference.huggingface.co/models/gpt2"; // Endpoint para o modelo gpt2
        this.huggingFaceApiKey = process.env.HUGGINGFACE_API_KEY || process.env.OPENAI_API_KEY; // Usar a mesma variável de ambiente por enquanto
        
        if (!this.huggingFaceApiKey) {
            console.warn("ATENÇÃO: Chave da API do Hugging Face (HUGGINGFACE_API_KEY ou OPENAI_API_KEY) não configurada. A IA generativa pode não funcionar.");
        }
    ...
    async generateResponse(channel, userstate, message) {
        try {
            ...
            const response = await axios.post(this.huggingFaceApiUrl, payload, { headers });

            const aiResponse = response.data[0].generated_text.replace(this.getSystemPrompt() + "\n" + prompt, "").trim();
            ...
            console.error("Erro ao gerar resposta da IA (Hugging Face):", error.response ? error.response.data : error.message);
        }
    }
    ...
    async generateGameplayComment(event) {
        try {
            ...
            const comment = response.data[0].generated_text.replace(this.getSystemPrompt() + "\n" + prompt, "").trim();
            ...
            console.error("Erro ao gerar comentário de gameplay (Hugging Face):", error.response ? error.response.data : error.message);
        }
    }
